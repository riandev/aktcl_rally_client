...
const Qc = () => {
  ...
  const handleSearch = () => {
    fetch(`http://192.168.200.10:5020/dMatched/${qcNumber}`)
      .then((res) => res.json())
      .then((data) => setMatchedQC(data));
  };
  const handleUpdate = (id) => {
    console.log(id);
    fetch(`http://192.168.200.10:5020/update/${id}`)
      .then((res) => res.json())
        ...
        <table className="table table-borderless table-hover">
          ...
          <tbody>
            <tr>
              ...
              <td>{matchedQC?.answer12}</td>
              <td>{matchedQC?.answer13}</td>
              <td>{matchedQC?.answer14}</td>
              <td>{matchedQC?.answer15}</td>
              <td>
                <button
