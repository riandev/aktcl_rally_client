...
const TerritoryPlusReport = () => {
  ...
  useEffect(() => {
    fetch("http://192.168.10.11:5020/territoryPlusReport")
      .then((res) => res.json())
      .then((data) => setPlusTerritory(data));
  }, []);
  const grandTarget = plusTerritory[0]?.grandSumTarget;
  const grandValidData = plusTerritory[0]?.grandSumValidData;
  const grandLessContacted = plusTerritory[0]?.grandSumLessContacted;
  const grandConnectedCall = plusTerritory[0]?.grandSumConnectedCall;
  const grandTruelyConnected = plusTerritory[0]?.grandSumTruelyConnected;
  const grandNotConnected = plusTerritory[0]?.grandSumNotConnected;
  const grandWtgNonSOB = plusTerritory[0]?.grandSumWtgNonSOB;
  const grandExtMSB = plusTerritory[0]?.grandSumExtMSB;
  const grandFalseContact = plusTerritory[0]?.grandSumFalseContact;
  const grandNoFreeSample = plusTerritory[0]?.grandSumNoFreeSample;
  const grandLessFreeSample = plusTerritory[0]?.grandSumLessFreeSample;
  const grandNoAndLessFreeSample =
    plusTerritory[0]?.grandSumNoAndLessFreeSample;
  const grandTeaSnacks = plusTerritory[0]?.grandSumTeaSnacks;
  const grandRetaination = plusTerritory[0]?.grandSumRetaination;
  const grandTargetTrueContact = plusTerritory[0]?.grandSumTargetTrueContact;
  const grandExtrapulatedData = plusTerritory[0]?.grandSumExtrapulatedData;
  const grandLessMoreTrueContacted =
    plusTerritory[0]?.grandSumLessMoreTrueContacted;
  const grandAvgConsumerAmount = plusTerritory[0]?.grandAvgConsumerAmount;
  const grandChargeAmount = plusTerritory[0]?.grandSumChargeAmount;
  let grand = parseFloat(grandChargeAmount).toFixed(2);
  const grandAvgConsumerCount = plusTerritory[0]?.grandAvgConsumerCount;
  return (
    <div className="m-3">
...
export default TerritoryPlusReport;
